...
class ApirateLimit
{
    ...
    {
    	// $this->pdo = new PDO('mysql:host=localhost;dbname=drivertesting;charset=utf8', 'root', '123456789');
    ...
    public function __invoke()
    {
        ...
        $this->originurl = $this->bypass_url()['REQUEST_URI'];
	...
	protected function mustbethrottled () {
        ...
        if($this->originip == MY_IP){
            return FALSE;
        }
        
        $sql = "SELECT count(*) as requests FROM ( select 1 FROM ".$this->table." WHERE originip = '".$this->originip."' AND url = '".$this->originurl."' AND ts >= date_sub(NOW(), interval ".$this->inmins." MINUTE)  LIMIT ".$this->requests." ) AS result";
        
        // $stmt = $this->pdo->prepare($sql);
        // $stmt->execute();
        $stmt = DB::SELECT($sql);

        // print_r($stmt[0]->requests);
        // die();
	...
	protected function info_about_ip() {
        ...
        // cloudfare ip ranges 
        //
        // https://www.cloudflare.com/ips-v4 30/10/2020
        // https://www.cloudflare.com/ips-v6 30/10/2020
        //    	
    ...
    protected function bypass_url() {
        ...
        foreach ($url_ranges as $range) {
            if ($range == $_SERVER['REQUEST_URI']) {
                break;
            }
        }	
